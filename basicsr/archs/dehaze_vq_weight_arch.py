    ...
    def forward(self, z, gt_indices=None, current_iter=None, weight_alpha=None):
        ...
        if self.use_weight and self.LQ_stage :
            if weight_alpha is not None:
                ...
                self.weight_alpha = weight_alpha
            d = d * (1 + self.weight * self.weight_alpha + self.weight * self.weight_alpha * self.weight * self.weight_alpha * 0.5)
...
class SwinLayers(nn.Module):
    ...
    def __init__(self, input_resolution=(32, 32), embed_dim=256, blk_depth=6, num_heads=8, window_size=8, **kwargs):
        ...
        for i in range(4):
            ...
            layer = RHAG(embed_dim, input_resolution, blk_depth, num_heads, window_size, patch_size=1,compress_ratio=3,
                 squeeze_factor=30,
                 conv_scale=0.01,
                 overlap_ratio=0.5, **kwargs)
...
class MultiScaleEncoder(nn.Module):
    ...
    def __init__(self, in_channel, max_depth, input_res=256, channel_query_dict=None, norm_type='gn',
                 act_type='leakyrelu', LQ_stage=True, **swin_opts):
...
class DecoderBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, norm_type='gn', act_type='leakyrelu'):
        ...
        self.block += [
            ...
            nn.Conv2d(in_channel, out_channel, 3, stride=1, padding=1),  # 卷积层
            ResBlock(out_channel, out_channel, norm_type, act_type),  # Residual Block
            ResBlock(out_channel, out_channel, norm_type, act_type),  # Residual Block
        ]
...
class WarpBlock(nn.Module):
    ...
    def __init__(self, in_channel):
        ...
        super().__init__()
        self.offset = nn.Conv2d(in_channel * 2, in_channel, 3, stride=1, padding=1)  # 偏移卷积层
        self.dcn = DCNv2Pack(in_channel, in_channel, 3, padding=1, deformable_groups=4)  # 可变形卷积层

    def forward(self, x_vq, x_residual):

        x_residual = self.offset(torch.cat([x_vq, x_residual], dim=1))  # 计算偏移
        feat_after_warp = self.dcn(x_vq, x_residual)  # 变形卷积

        return feat_after_warp  #After NFA


class MultiScaleDecoder(nn.Module):

    def __init__(self, in_channel, max_depth, input_res=256, channel_query_dict=None, norm_type='gn',
                 act_type='leakyrelu', only_residual=False, use_warp=True):
        ...
        for i in range(max_depth):
            ...
            self.warp.append(WarpBlock(out_channel))# NFA
            res = res * 2
        ...
        for idx, m in enumerate(self.upsampler):
            with torch.backends.cudnn.flags(enabled=False):
                if not self.only_residual:
                    x = m(x)
                    if self.use_warp:
                        x_vq = self.warp[idx](code_decoder_output[idx], x)  # NFA
                        # print(idx, x.mean(), x_vq.mean())
                        x = x + x_vq * (x.mean() / x_vq.mean())  # 残差连接
                    else:
                        x = x + code_decoder_output[idx]  # 残差连接
                else:
                    x = m(x)
        # print()
        return x

@ARCH_REGISTRY.register() #是 Python 中的装饰器语法，用于将一个函数或类注册到指定的注册表中，以便后续可以通过名称或其他标识符来访问。
