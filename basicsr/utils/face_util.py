...
"""
这段代码定义了一个用于人脸修复的辅助类 `FaceRestorationHelper`，主要包含以下功能：

- `init_dlib(detection_path, landmark5_path, landmark68_path)`: 初始化 dlib 检测器和预测器，用于检测人脸和定位人脸关键点。需要提供
检测器和关键点定位器的路径。

- `free_dlib_gpu_memory()`: 释放 dlib 占用的 GPU 内存。

- `read_input_image(img_path)`: 从文件加载输入图像。

- `detect_faces(img_path, upsample_num_times=1, only_keep_largest=False)`: 在图像中检测人脸。可选择是否对图像进行上采样，并且可以
选择仅保留最大的人脸。

- `get_face_landmarks_5()`: 获取检测到的人脸的五个关键点。

- `get_face_landmarks_68()`: 获取检测到的人脸的 68 个关键点。

- `warp_crop_faces(save_cropped_path=None, save_inverse_affine_path=None)`: 对检测到的人脸进行仿射变换、裁剪和保存。还获取了逆仿射
矩阵以备后续处理。

- `add_restored_face(face)`: 添加修复后的人脸。

- `paste_faces_to_input_image(save_path)`: 将修复后的人脸粘贴回原始图像中，并保存。
